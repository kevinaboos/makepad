...
impl Font {
    ...
    pub fn glyph_outline(&self, glyph_id: GlyphId) -> Option<GlyphOutline> {
        if let Some(outline) = self.cached_glyph_outlines.borrow().get(&glyph_id).cloned() {
            return outline;
        }
    ...
    pub fn glyph_outline_bounds_in_ems(
        ...
        if let Some(cached_outline) = self.cached_glyph_outlines.borrow().get(&glyph_id).cloned() {
            *out_outline = cached_outline.clone();
            return cached_outline.map(|outline| outline.bounds_in_ems());
        }
